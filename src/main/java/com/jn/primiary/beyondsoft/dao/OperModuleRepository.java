...
public interface OperModuleRepository extends JpaRepository<OperStandard, Integer> {
    ...
    @Query(value = "select code,name,createPerson,createTime, isAuth,updateTime,id,batchNo,dataType,categoryId,count from ( " +
            ...
            "date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,is_auth as isAuth," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,id as id,batch_no as batchNo," +
            ...
            "union " +
            "select schemacode as code,schemaname as name,creator as createPerson," +
            "date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,'1' as isAuth," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,id as id,null as batchNo," +
            "null as dataType,categoryid as categoryId,version as version,count as count from tb_stdgl_schemamodule a where status='1'" +
            ") a where categoryid=?1 " +
    ...
    List<Object[]> getAllStandardInfo2();
    ...
    @Query(value = "select code,name,createPerson,createTime, isAuth,updateTime,id,batchNo,dataType,categoryId,count from ( " +
            ...
            ") a " +
            "order by isAuth desc,createTime desc", nativeQuery = true)
    List<Object[]> getAllStandardInfo();

    @Query(value = "select code,name,createPerson,createTime, isAuth,updateTime,id,batchNo,dataType,categoryId from ( " +
            "select schemacode as code,schemaname as name,creator as createPerson," +
            "date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,'1' as isAuth," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,id as id,' ' as batchNo," +
            "' ' as dataType,categoryid as categoryId,version as version from tb_stdgl_schemamodule a where status='1'" +
            ") b where  categoryid=?1 order by isAuth desc,createTime desc", nativeQuery = true)
    ...
    @Query(value = "select code,name,createPerson,createTime, isAuth,updateTime,id,batchNo,dataType,categoryId from ( " +
            ...
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,id as id,' ' as batchNo," +
            "' ' as dataType,categoryid as categoryId,version as version from tb_stdgl_schemamodule a where status='1'" +
            ") b order by isAuth desc,createTime desc", nativeQuery = true)
    List<Object[]> getAllSHTGStandardInfo2();
...
//            "date_format (a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,a.is_auth as isAuth," +
...
//    List<Object[]> getOperStandardAndCodeTable(String dbId);
    ...
    @Query(value = "select a.id as id,a.batch_no as batchNo,a.schemacode as code,a.schemaname as name," +
            "a.enname as enName,a.creator as createPerson," +
            "CAST(a.createtime AS char) as createTime,a.is_auth as isAuth," +
            "a.data_type as dataType,a.categoryid as categoryId,a.db_id as db_id,a.version as version " +
            "from oper_tb_stdgl_schemamodule a where db_id = ?1 and status = '1'" +
            "union " +
            "select b.id as id,b.batch_no as batchNo,b.codetable_ename as code,b.codetable_cname as name," +
            "b.codetable_ename as enName,b.create_user as createPerson," +
            "CAST(b.create_time AS char) as createTime,b.is_auth as isAuth," +
            "b.data_type as dataType," +
            "' ' as categoryId,b.db_id as db_id,' ' as version from oper_tb_stdgl_code_info b where b.db_id = ?1 and status = '1' group by b.codetable_ename", nativeQuery = true)
    List<Object[]> getOperStandardAndCodeTable(String dbId);
    ...
    @Query(value = "select enname as ename,schemaname as cname,schemacode as code," +
            "date_format(createtime,'%Y-%m-%d %H:%i:%S') as createTime,creator as createUser,is_auth as isAuth," +
            "audit_person as auditUser,date_format(audit_time,'%Y-%m-%d %H:%i:%S') as auditTime," +
    ...
    @Query(value = "select date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,a.* from oper_tb_stdgl_schemamodule a " +
    ...
    @Query(value = "select count(*) from ( " +
            ...
            "null as dataType,categoryid as categoryId,version as version from tb_stdgl_schemamodule a where status='1'" +
            ") a ", nativeQuery = true)
    Integer getAllStandardInfoCount();
    ...
    /*@Query(value ="select code,name,createPerson,createTime, isAuth,updateTime,id,batchNo,dataType,categoryId,count from ( " +
            ...
            "date_format(std.createtime,'%Y-%m-%d %H:%i:%S') as createTime,'1' as isAuth," +
            "date_format(std.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,std.id as id,null as batchNo," +
    ...
    @Query(value = "select code,name,createPerson,createTime, isAuth,updateTime,id,batchNo,dataType,categoryId,count from ( " +
            ...
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,id as id,batch_no as batchNo," +
            "data_type as dataType,categoryid as categoryId,version as version,'0' as count from oper_tb_stdgl_schemamodule a where is_auth in ('3','5') " +
            "and   (schemacode like CONCAT('%',?1,'%') or schemaname like CONCAT('%',?1,'%') or enname like CONCAT('%',?1,'%')) " +
//            "or (id in (select moudle_id from oper_tb_stdgl_schemafield where fieldname like CONCAT('%',?1,'%') or enname like CONCAT('%',?1,'%') or fieldcode like CONCAT('%',?1,'%') )))"+
            "union " +
            "select schemacode as code,schemaname as name,creator as createPerson," +
            "date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,'1' as isAuth," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,id as id,null as batchNo," +
            "null as dataType,categoryid as categoryId,version as version,count as count from tb_stdgl_schemamodule a where status='1' " +
            "and   (schemacode like CONCAT('%',?1,'%') or schemaname like CONCAT('%',?1,'%') or enname like CONCAT('%',?1,'%') ) " +
//            "or (id in (select moudle_id from tb_stdgl_schemafield where fieldname like CONCAT('%',?1,'%') or enname like CONCAT('%',?1,'%') or fieldcode like CONCAT('%',?1,'%') )))"+
            ") a order by isAuth asc,createTime desc  ", nativeQuery = true)
    List<Object[]> searchStandard(String searchword);

    @Query(value = "select distinct code,name,createPerson,createTime, isAuth,updateTime,id, batchNo,dataType,categoryId,count from ( " +
            "select schemacode as code,schemaname as name,a.creator as createPerson," +
            "date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,is_auth as isAuth," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,a.id as id,a.batch_no as batchNo," +
            "a.data_type as dataType,categoryid as categoryId,a.version as version,'0' as count from oper_tb_stdgl_schemamodule a left join oper_tb_stdgl_schemafield b " +
            "on a.id = b.moudle_id where a.is_auth in ('3','5') " +
            "and (b.fieldname like CONCAT('%',?1,'%') or b.enname like CONCAT('%',?1,'%') or b.fieldcode like CONCAT('%',?1,'%')) " +
            "union " +
            "select schemacode as code,schemaname as name,a.creator as createPerson," +
            "date_format(a.createtime,'%Y-%m-%d %H:%i:%S') as createTime,'1' as isAuth," +
            "date_format(a.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,a.id as id,null as batchNo," +
            "null as dataType,categoryid as categoryId,a.version as version,count as count from tb_stdgl_schemamodule a left join tb_stdgl_schemafield b " +
            "on a.id = b.moudle_id where a.status='1' " +
            "and (b.fieldname like CONCAT('%',?1,'%') or b.enname like CONCAT('%',?1,'%') or b.fieldcode like CONCAT('%',?1,'%')) " +
            ") a order by isAuth asc,createTime desc  ", nativeQuery = true)
    List<Object[]> searchStandard2(String searchword);
    /*@Query(value ="select count(*) from ( " +
            "select std.schemacode as code,std.schemaname as name,std.creator as createPerson," +
            "date_format(std.createtime,'%Y-%m-%d %H:%i:%S') as createTime,'1' as isAuth," +
            "date_format(std.update_time,'%Y-%m-%d %H:%i:%S') as updateTime,std.id as id,null as batchNo," +
