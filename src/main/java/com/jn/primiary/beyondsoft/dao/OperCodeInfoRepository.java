...
public interface OperCodeInfoRepository extends JpaRepository <OperCodeInfo, Integer> {
    ...
    @Query(value = "select " +
            ...
            " from oper_tb_stdgl_code_info a where codetable_ename=?1 and batch_no=?2 and status='1'",nativeQuery = true)
    ...
    @Query(value = "select distinct(codetable_ename) as ename,codetable_cname as cname,codetable_cname as code," +
            ...
            "audit_user as auditUser,date_format(audit_time,'%Y-%m-%d %h:%m:01') as auditTime," +
    ...
    @Query(value = "select codetableEname,codetableCname,createTime,createUser,isAuth,batchNo,dataType from (" +
            ...
            "'1' as isAuth,null as batchNo,null as dataType from tb_stdgl_code_info a " +
            "where a.status='1'" +
            ") tmp where tmp.codetableCname like CONCAT('%',:searchword,'%') or tmp.codetableEname like CONCAT('%',:searchword,'%') " +
            "order by isAuth asc,createTime desc limit :startindex,:pagesize",nativeQuery = true)
    ...
    @Query(value = "select count(*) from (" +
            ...
            "where a.status='1'" +
            ") tmp where tmp.codetableCname like CONCAT('%',:searchword,'%') or tmp.codetableEname like CONCAT('%',:searchword,'%')  " +
            "order by isAuth asc,createTime desc",nativeQuery = true)
